"""
Script to add 6 dummy products to the ecommerce section
Run with: python seed_dummy_products.py
"""
...
with app.app_context():
    ...
    if existing_count > 0:
        print(f"[INFO] {existing_count} products already exist. Skipping seed.")
    else:
        ...
        # Dummy products data - 6 agricultural products
        dummy_products = [
            {
        ...
        # Create products
        for i, product_data in enumerate(dummy_products, 1):
            created_at = datetime.utcnow() - timedelta(days=i*2)  # Spread over 12 days
            ...
            print(f"[{i}/6] Created: {product_data['title']}")
        
        db.session.commit()
        print(f"\n[SUCCESS] Created {len(dummy_products)} dummy products!")
